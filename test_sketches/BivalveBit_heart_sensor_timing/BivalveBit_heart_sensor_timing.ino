/* BivalveBit_heart_sensor_timing.ino
 *  
 ...
 *  
 *  Based on the timing below, it takes about 23 milliseconds to go through 
 *  a cycle of turning on the VREG, re-setting the MAX3010x sensor, clearing
 *  the FIFO buffer, getting a new reading, and shutting down the VREG.
 *  
 * 
 */
...
//    digitalWrite(REDLED,HIGH); // set high to turn off
  ...
  if (maxFlag) {
     // If the sensor responded and started up, adjust its settings
...
void loop() {
  ...
  if ( (millis() - myMillis) >= myIntervalMS) {
    ...
    myMillis = millis(); // Start of timing section
    ...
    // Re-enable the MAX3010x settings and clear the sensor's FIFO buffer 
    heartBuffer[0] = restartAndSampleMAX3010x(max3010x, IRledBrightness, sampleAverage, ledMode, sampleRate, pulseWidth, adcRange, REDledBrightness);
    Serial.println(heartBuffer[0]);

    digitalWrite(VREG_EN, LOW); // set low to turn off, high to turn on (~150usec to wake)
    
//    Serial.print("Cycle time: "); Serial.println(millis() - myMillis);  // end of timing section, takes about 23ms to get here
  }
 ...
 ************************************/
uint32_t restartAndSampleMAX3010x(MAX30105 &max3010x, byte IRledBrightness, byte sampleAverage, byte ledMode, int sampleRate, int pulseWidth, int adcRange, byte REDledBrightness)
{
